...
import '../quick_blox_repository/message_wrapper.dart';
...
class QuickBloxService extends ChangeNotifier{
  ...
  getInstance(){
    if(_pref!.getInt(GwcApi.qbCurrentUserId) != null){
      _localUserId = _pref!.getInt(GwcApi.qbCurrentUserId);
    }
  }
  ...
  Future<bool> getSession() async{
    bool? _isExpired;
    int? expiry = _pref!.getInt(GwcApi.getQBSession);
    if(expiry != null){
  ...
  Future<bool> login(String userName) async {
    bool isLogin;
    try {
      QBLoginResult result = await QB.auth.login(userName, defaultPassword);

      QBUser? qbUser = result.qbUser;
      _localUserId = qbUser?.id ?? -1;
      connect(qbUser!.id!);
      // on login success store username, password, userid to local
      _pref!.setInt(GwcApi.qbCurrentUserId, _localUserId!);

      QBSession? _qbSession = result.qbSession;
      qbSession = _qbSession;
      isLogin = true;
      _pref!.setBool(GwcApi.isQBLogin, true);
      _pref!.setInt(GwcApi.getQBSession, DateTime.parse(_qbSession!.expirationDate!).millisecondsSinceEpoch);
      print("login success..");
  ...
  Future<void> logout() async {
    try {
      ...
      _pref!.setBool(GwcApi.isQBLogin, false);
      _pref!.remove(GwcApi.getQBSession);
      _pref!.remove(GwcApi.qbCurrentUserId);
  ...
  connect(int userId) async {
    try {
      await QB.chat.connect(userId, defaultPassword);
  ...
  Future joinDialog(String dialogId) async {
    try {
      ...
      print("join room error: ${dialogId}");
  ...
  Future<List<QBMessageWrapper>> _wrapMessages(List<QBMessage?> messages) async {
    ...
    for (QBMessage? message in messages) {
      ...
      print(_localUserId);
      wrappedMessages.add(QBMessageWrapper(senderName, message, _pref!.getInt(GwcApi.qbCurrentUserId)!));
    }
  ...
  void markMessageRead() async {
    ...
    qbMessage.senderId = (_localUserId == null) ? _pref!.getInt(GwcApi.qbCurrentUserId) : _localUserId;
